...
struct hv_device *fuzzed_stor_dev;

struct vmbus_channel *get_stor_channel()
{
	struct storvsc_device *stor_device;
	u16 q_num;

	stor_device = hv_get_drvdata(fuzzed_stor_dev);
	q_num = get_cpu();
	
	return stor_device->stor_chns[q_num];
}
EXPORT_SYMBOL_GPL(get_stor_channel);
