...
static int hv_kbd_connect_to_vsp(struct hv_device *hv_dev)
{
	...
	printk(KERN_ALERT "Before calling = %d\n", proto_status);
	...
	response = &kbd_dev->protocol_resp;
	proto_status = __le32_to_cpu(response->proto_status);
	printk(KERN_ALERT "After calling = %d\n", proto_status);
...
int vmfuzz_keyboard()
{
	struct hv_kbd_dev *kbd_dev = hv_get_drvdata(fuzzed_dev);
	struct synth_kbd_protocol_request *input;
	struct synth_kbd_protocol_response *response;
	u32 proto_status;
	int error;

	input = &kbd_dev->protocol_req;
	memset(input, 0, sizeof(struct synth_kbd_protocol_request));

	//printk(KERN_ALERT "fuzzed_dev, hv_dev = %x\n", fuzzed_dev);
	//printk(KERN_ALERT "kbd_dev = %x\n", kbd_dev);

	//printk(KERN_ALERT "Wrong Message\n");
	input->header.type = __cpu_to_le32(2);
	input->version_requested.version = __cpu_to_le32(2352);
	
	response = &kbd_dev->protocol_resp;
	response->proto_status = 0;
	
	printk(KERN_ALERT "Before calling = %d\n", proto_status);
	error = vmbus_sendpacket(fuzzed_dev->channel, input,
				 sizeof(struct synth_kbd_protocol_request),
				 (unsigned long)input,
				 VM_PKT_DATA_INBAND,
				 VMBUS_DATA_PACKET_FLAG_COMPLETION_REQUESTED);
	...
	printk(KERN_ALERT "After calling = %d\n", proto_status);
	if (!(proto_status & PROTOCOL_ACCEPTED)) {
		printk(KERN_ALERT "synth_kbd protocol request failed (version %d)\n",
		        SYNTH_KBD_VERSION);
	} else
	{
		printk(KERN_ALERT "synth_kbd protocol request success (version %d)\n",
		        SYNTH_KBD_VERSION);
	}

	int i=0, j=0;
	__le32 type[10], version[10];
	for(i=0; i<10; i++)
	{
		for(j=0; j<10; j++)
		{
			// request and response have a same structure [header||__le32]
			// type = (u32)random % 6
			// version = (u32)random
			get_random_bytes(type+j, sizeof(__le32));
			get_random_bytes(version+j, sizeof(__le32));
			printk(KERN_ALERT "[keyboard] [%x|%08x]\n",
				type[j] % 6, version[j]);
		}

		for(j=0; j<10; j++)
		{
			input->header.type = __cpu_to_le32(type[j] % 6);
			input->version_requested.version = __cpu_to_le32(version[j]);
			error = vmbus_sendpacket(fuzzed_dev->channel, input,
						 sizeof(struct synth_kbd_protocol_request),
						 (unsigned long)input,
						 VM_PKT_DATA_INBAND,
